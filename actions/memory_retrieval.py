...
def retrieve_recent_memory(memory: Memory, n: int = 5) -> List[Dict[str, Any]]:
    ...
    Args:
        memory (Memory): The memory instance from which to retrieve recent items.
...
def retrieve_task_result(memory: Memory, task_id: str) -> Dict[str, Any]:
    """
    Retrieve the result of a specific task from memory.

    Args:
        memory (Memory): The memory instance from which to retrieve the task result.
        task_id (str): The unique identifier of the task whose result is to be retrieved.

    Returns:
        Dict[str, Any]: A dictionary containing the result of the task.
    """
    return memory.get_task_result(task_id)
...
def retrieve_context_for_task(memory: Memory, task_description: str, n: int = 5) -> Dict[str, Any]:
    ...
    Args:
        memory (Memory): The memory instance from which to retrieve context.
...
def query_long_term_memory(memory: Memory, query: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    Args:
        memory (Memory): The memory instance to query.
